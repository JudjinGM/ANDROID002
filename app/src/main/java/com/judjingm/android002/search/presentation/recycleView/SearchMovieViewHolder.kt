...
class SearchMovieViewHolder(
    ...
    private val onItemClicked: (SearchContentUiItem.MovieUiItem) -> Unit
) : RecyclerView.ViewHolder(binding.root) {

    fun bind(content: SearchContentUiItem.MovieUiItem) {
        binding.titleTextView.text = content.title
        binding.voteTextView.text = content.averageVote
        binding.yearTextView.text = content.releaseDate
        Glide
            .with(itemView)
