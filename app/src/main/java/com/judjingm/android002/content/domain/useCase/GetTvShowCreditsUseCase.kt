...
interface GetTvShowCreditsUseCase {
    fun execute(seriesId: Int): Flow<Resource<Credits, ErrorEntity>>

    class Base @Inject constructor(private val repository: ContentDetailsRepository) :
        GetTvShowCreditsUseCase {
        override fun execute(seriesId: Int): Flow<Resource<Credits, ErrorEntity>> {
            return repository.getTvShowCredits(seriesId)
        }
    }
